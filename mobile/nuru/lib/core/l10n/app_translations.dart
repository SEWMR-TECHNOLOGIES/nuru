class AppTranslations {
  static const Map<String, Map<String, String>> _translations = {
    // ── App ──
    'app_name': {'en': 'Nuru', 'sw': 'Nuru'},
    'app_tagline': {
      'en': 'Keep your event together',
      'sw': 'Weka tukio lako pamoja',
    },

    // ── Onboarding ──
    'onboarding_title_1': {
      'en': 'Keep your event\ntogether',
      'sw': 'Weka tukio lako\npamoja',
    },
    'onboarding_desc_1': {
      'en':
          'Track contributions, coordinate guests, and work with trusted vendors from one clear place.',
      'sw':
          'Fuatilia michango, ratibu wageni, na fanya kazi na watoa huduma waaminifu sehemu moja.',
    },
    'onboarding_title_2': {
      'en': 'Every event type\none place',
      'sw': 'Kila aina ya tukio\nsehemu moja',
    },
    'onboarding_desc_2': {
      'en':
          'Weddings, corporate events, birthdays, memorials, and more. Nuru keeps everything together in one place.',
      'sw':
          'Harusi, matukio ya biashara, birthday, misiba, na zaidi. Nuru inaweka kila kitu pamoja sehemu moja.',
    },
    'onboarding_title_3': {
      'en': 'See everything\nthat matters',
      'sw': 'Ona kila kitu\nchenye umuhimu',
    },
    'onboarding_desc_3': {
      'en':
          'Contributions, guests, vendors, and progress. Keep every important detail clear and organized.',
      'sw':
          'Michango, wageni, watoa huduma, na maendeleo. Weka kila undani muhimu wazi na kwa mpangilio.',
    },
    'get_started': {'en': 'Get Started', 'sw': 'Anza Sasa'},
    'skip': {'en': 'Skip for now', 'sw': 'Ruka kwa sasa'},
    'next': {'en': 'Next', 'sw': 'Endelea'},

    // ── Auth ──
    'welcome_back': {'en': 'Welcome\nBack', 'sw': 'Karibu\nTena'},
    'sign_in_subtitle': {
      'en': 'Sign in to keep your events organized',
      'sw': 'Ingia ili kuweka matukio yako kwa mpangilio',
    },
    'create_account': {'en': 'Create\nAccount', 'sw': 'Fungua\nAkaunti'},
    'sign_up_subtitle': {
      'en': 'Join Nuru and start organizing your events with clarity',
      'sw': 'Jiunge na Nuru na uanze kupanga matukio yako kwa uwazi',
    },
    'email': {'en': 'Email', 'sw': 'Email'},
    'password': {'en': 'Password', 'sw': 'Password'},
    'full_name': {'en': 'Full Name', 'sw': 'Jina Kamili'},
    'first_name': {'en': 'First Name', 'sw': 'Jina la Kwanza'},
    'last_name': {'en': 'Last Name', 'sw': 'Jina la Mwisho'},
    'phone': {'en': 'Phone Number', 'sw': 'Namba ya Simu'},
    'username': {'en': 'Username', 'sw': 'Username'},
    'sign_in': {'en': 'Sign In', 'sw': 'Ingia'},
    'sign_up': {'en': 'Sign Up', 'sw': 'Jisajili'},
    'forgot_password': {'en': 'Forgot Password?', 'sw': 'Umesahau Password?'},
    'dont_have_account': {
      'en': "Don't have an account? ",
      'sw': 'Huna akaunti? ',
    },
    'already_have_account': {
      'en': 'Already have an account? ',
      'sw': 'Una akaunti tayari? ',
    },
    'or_continue_with': {'en': 'or continue with', 'sw': 'au endelea na'},
    'terms_agree': {
      'en': 'By signing up, you agree to our ',
      'sw': 'Kwa kujisajili, unakubali ',
    },
    'terms_of_service': {'en': 'Terms of Service', 'sw': 'Masharti ya Huduma'},
    'and': {'en': ' and ', 'sw': ' na '},
    'privacy_policy': {'en': 'Privacy Policy', 'sw': 'Sera ya Faragha'},
    'sign_out': {'en': 'Sign Out', 'sw': 'Toka'},
    'log_out': {'en': 'Log Out', 'sw': 'Toka'},
    'login': {'en': 'Login', 'sw': 'Ingia'},
    'register': {'en': 'Register', 'sw': 'Jisajili'},
    'verify': {'en': 'Verify', 'sw': 'Thibitisha'},
    'verification': {'en': 'Verification', 'sw': 'Uthibitisho'},
    'enter_otp': {
      'en': 'Enter the verification code',
      'sw': 'Ingiza nambari ya uthibitisho',
    },
    'resend_code': {'en': 'Resend Code', 'sw': 'Tuma Tena Nambari'},
    'reset_password': {'en': 'Reset Password', 'sw': 'Weka Password Mpya'},
    'new_password': {'en': 'New Password', 'sw': 'Password Mpya'},
    'confirm_password': {'en': 'Confirm Password', 'sw': 'Thibitisha Password'},
    'phone_or_email': {'en': 'Phone or Email', 'sw': 'Simu au Email'},
    'continue_text': {'en': 'Continue', 'sw': 'Endelea'},

    // ── Home ──
    'home': {'en': 'Home', 'sw': 'Nyumbani'},
    'feed': {'en': 'Feed', 'sw': 'Feed'},
    'explore': {'en': 'Explore', 'sw': 'Gundua'},
    'search': {'en': 'Search', 'sw': 'Tafuta'},
    'search_events': {'en': 'Search events...', 'sw': 'Tafuta matukio...'},
    'trending': {'en': 'Trending', 'sw': 'Trending'},
    'for_you': {'en': 'For You', 'sw': 'Kwako'},
    'discover': {'en': 'Discover', 'sw': 'Gundua'},
    'joined': {'en': 'Joined', 'sw': 'Umejiunga'},
    'notifications': {'en': 'Notifications', 'sw': 'Arifa'},
    'messages': {'en': 'Messages', 'sw': 'Ujumbe'},

    // ── Events ──
    'events': {'en': 'Events', 'sw': 'Matukio'},
    'my_events': {'en': 'My Events', 'sw': 'Matukio Yangu'},
    'create_event': {'en': 'Create Event', 'sw': 'Unda Tukio'},
    'edit_event': {'en': 'Edit Event', 'sw': 'Hariri Tukio'},
    'delete_event': {'en': 'Delete Event', 'sw': 'Futa Tukio'},
    'event_details': {'en': 'Event Details', 'sw': 'Maelezo ya Tukio'},
    'event_title': {'en': 'Event Title', 'sw': 'Jina la Tukio'},
    'event_type': {'en': 'Event Type', 'sw': 'Aina ya Tukio'},
    'event_date': {'en': 'Event Date', 'sw': 'Tarehe ya Tukio'},
    'event_time': {'en': 'Event Time', 'sw': 'Wakati wa Tukio'},
    'event_location': {'en': 'Location', 'sw': 'Mahali'},
    'venue': {'en': 'Venue', 'sw': 'Mahali pa Tukio'},
    'description': {'en': 'Description', 'sw': 'Maelezo'},
    'start_date': {'en': 'Start Date', 'sw': 'Tarehe ya Kuanza'},
    'end_date': {'en': 'End Date', 'sw': 'Tarehe ya Mwisho'},
    'start_time': {'en': 'Start Time', 'sw': 'Wakati wa Kuanza'},
    'end_time': {'en': 'End Time', 'sw': 'Wakati wa Mwisho'},
    'dress_code': {'en': 'Dress Code', 'sw': 'Dress Code'},
    'budget': {'en': 'Budget', 'sw': 'Bajeti'},
    'guests': {'en': 'Guests', 'sw': 'Wageni'},
    'guest_list': {'en': 'Guest List', 'sw': 'Orodha ya Wageni'},
    'add_guest': {'en': 'Add Guest', 'sw': 'Ongeza Mgeni'},
    'invite_guests': {'en': 'Invite Guests', 'sw': 'Alika Wageni'},
    'rsvp': {'en': 'RSVP', 'sw': 'RSVP'},
    'confirmed': {'en': 'Confirmed', 'sw': 'Imethibitishwa'},
    'pending': {'en': 'Pending', 'sw': 'Inasubiri'},
    'declined': {'en': 'Declined', 'sw': 'Imekataliwa'},
    'attending': {'en': 'Attending', 'sw': 'Nitahudhuria'},
    'not_attending': {'en': 'Not Attending', 'sw': 'Sitahudhuria'},
    'maybe': {'en': 'Maybe', 'sw': 'Labda'},
    'upcoming': {'en': 'Upcoming', 'sw': 'Yanakuja'},
    'past': {'en': 'Past', 'sw': 'Yaliyopita'},
    'ongoing': {'en': 'Ongoing', 'sw': 'Yanaendelea'},
    'cancelled': {'en': 'Cancelled', 'sw': 'Yameghairiwa'},
    'published': {'en': 'Published', 'sw': 'Imechapishwa'},
    'draft': {'en': 'Draft', 'sw': 'Rasimu'},

    // ── Event Types ──
    'wedding': {'en': 'Wedding', 'sw': 'Harusi'},
    'birthday': {'en': 'Birthday', 'sw': 'Birthday'},
    'corporate': {'en': 'Corporate', 'sw': 'Corporate'},
    'memorial': {'en': 'Memorial', 'sw': 'Memorial'},
    'anniversary': {'en': 'Anniversary', 'sw': 'Anniversary'},
    'conference': {'en': 'Conference', 'sw': 'Conference'},
    'graduation': {'en': 'Graduation', 'sw': 'Graduation'},
    'sendoff': {'en': 'Send-off', 'sw': 'Send-off'},

    // ── Contributions ──
    'contributions': {'en': 'Contributions', 'sw': 'Michango'},
    'contribute': {'en': 'Contribute', 'sw': 'Changia'},
    'contributors': {'en': 'Contributors', 'sw': 'Wachangiaji'},
    'amount': {'en': 'Amount', 'sw': 'Kiasi'},
    'pledge': {'en': 'Pledge', 'sw': 'Ahadi'},
    'pledges': {'en': 'Pledges', 'sw': 'Ahadi'},
    'total_contributions': {
      'en': 'Total Contributions',
      'sw': 'Jumla ya Michango',
    },

    // ── Tickets ──
    'tickets': {'en': 'Tickets', 'sw': 'Tiketi'},
    'my_tickets': {'en': 'My Tickets', 'sw': 'Tiketi Zangu'},
    'buy_ticket': {'en': 'Buy Ticket', 'sw': 'Nunua Tiketi'},
    'ticket_price': {'en': 'Ticket Price', 'sw': 'Bei ya Tiketi'},
    'free': {'en': 'Free', 'sw': 'Bure'},
    'sold_out': {'en': 'Sold Out', 'sw': 'Zimeisha'},
    'available': {'en': 'Available', 'sw': 'Zinapatikana'},

    // ── Vendors / Services ──
    'vendors': {'en': 'Vendors', 'sw': 'Vendors'},
    'all': {'en': 'All', 'sw': 'Zote'},
    'people': {'en': 'People', 'sw': 'Watu'},
    'services': {'en': 'Services', 'sw': 'Huduma'},
    'my_services': {'en': 'My Services', 'sw': 'Huduma Zangu'},
    'find_services': {'en': 'Find Vendors', 'sw': 'Tafuta Vendors'},
    'book_service': {'en': 'Book Vendor', 'sw': 'Weka Huduma'},
    'bookings': {'en': 'Bookings', 'sw': 'Bookings'},
    'my_bookings': {'en': 'My Bookings', 'sw': 'Bookings Zangu'},
    'booked': {'en': 'Booked', 'sw': 'Imewekwa'},
    'booking_details': {'en': 'Booking Details', 'sw': 'Maelezo ya Booking'},

    // ── Profile ──
    'profile': {'en': 'Profile', 'sw': 'Wasifu'},
    'edit_profile': {'en': 'Edit Profile', 'sw': 'Hariri Wasifu'},
    'bio': {'en': 'Bio', 'sw': 'Bio'},
    'location': {'en': 'Location', 'sw': 'Mahali'},
    'website': {'en': 'Website', 'sw': 'Website'},
    'followers': {'en': 'Followers', 'sw': 'Wafuasi'},
    'following': {'en': 'Following', 'sw': 'Unawafuata'},
    'follow': {'en': 'Follow', 'sw': 'Fuata'},
    'unfollow': {'en': 'Unfollow', 'sw': 'Acha Kufuata'},
    'moments': {'en': 'Moments', 'sw': 'Moments'},
    'posts': {'en': 'Posts', 'sw': 'Machapisho'},

    // ── Settings ──
    'settings': {'en': 'Settings', 'sw': 'Mipangilio'},
    'account': {'en': 'Account', 'sw': 'Akaunti'},
    'preferences': {'en': 'Preferences', 'sw': 'Mapendeleo'},
    'change_password': {'en': 'Change Password', 'sw': 'Badilisha Password'},
    'identity_verification': {
      'en': 'Identity Verification',
      'sw': 'Uthibitisho wa Kitambulisho',
    },
    'verify_identity': {
      'en': 'Verify your identity for trust',
      'sw': 'Thibitisha kitambulisho chako kwa uaminifu',
    },
    'privacy_security': {
      'en': 'Privacy & Security',
      'sw': 'Faragha na Usalama',
    },
    'notification_settings': {'en': 'Notifications', 'sw': 'Arifa'},
    'about': {'en': 'About', 'sw': 'Kuhusu'},
    'about_nuru': {'en': 'About Nuru', 'sw': 'Kuhusu Nuru'},
    'version': {'en': 'Version', 'sw': 'Toleo'},
    'terms_and_licenses': {
      'en': 'Version, terms, and licenses',
      'sw': 'Toleo, masharti, na leseni',
    },
    'update_personal_info': {
      'en': 'Update your personal information',
      'sw': 'Sasisha taarifa zako binafsi',
    },
    'update_account_password': {
      'en': 'Update your account password',
      'sw': 'Sasisha password ya akaunti yako',
    },
    'manage_notifications': {
      'en': 'Choose what notifications you receive',
      'sw': 'Chagua arifa unazopokea',
    },
    'control_privacy': {
      'en': 'Control your privacy settings',
      'sw': 'Dhibiti mipangilio yako ya faragha',
    },
    'language': {'en': 'Language', 'sw': 'Lugha'},
    'choose_language': {
      'en': 'Choose your preferred language',
      'sw': 'Chagua lugha unayopendelea',
    },
    'english': {'en': 'English', 'sw': 'Kiingereza'},
    'swahili': {'en': 'Swahili', 'sw': 'Kiswahili'},
    'dark_mode': {'en': 'Dark Mode', 'sw': 'Hali ya Giza'},
    'light_mode': {'en': 'Light Mode', 'sw': 'Hali ya Mwanga'},

    // ── Actions ──
    'save': {'en': 'Save', 'sw': 'Hifadhi'},
    'cancel': {'en': 'Cancel', 'sw': 'Ghairi'},
    'delete': {'en': 'Delete', 'sw': 'Futa'},
    'edit': {'en': 'Edit', 'sw': 'Hariri'},
    'close': {'en': 'Close', 'sw': 'Funga'},
    'done': {'en': 'Done', 'sw': 'Imekamilika'},
    'confirm': {'en': 'Confirm', 'sw': 'Thibitisha'},
    'submit': {'en': 'Submit', 'sw': 'Tuma'},
    'send': {'en': 'Send', 'sw': 'Tuma'},
    'share': {'en': 'Share', 'sw': 'Shiriki'},
    'copy': {'en': 'Copy', 'sw': 'Nakili'},
    'download': {'en': 'Download', 'sw': 'Pakua'},
    'upload': {'en': 'Upload', 'sw': 'Pakia'},
    'refresh': {'en': 'Refresh', 'sw': 'Onyesha upya'},
    'retry': {'en': 'Try again', 'sw': 'Jaribu Tena'},
    // ── Right Sidebar ──
    'quick_view': {'en': 'Quick View', 'sw': 'Muhtasari'},
    'upcoming_events': {'en': 'Upcoming Events', 'sw': 'Matukio Yanayokuja'},
    'my_meetings': {'en': 'My Meetings', 'sw': 'Mikutano Yangu'},
    'events_with_tickets': {
      'en': 'Events with Tickets',
      'sw': 'Matukio yenye Tiketi',
    },
    'service_providers': {'en': 'Service Providers', 'sw': 'Watoa Huduma'},
    'suggested_for_you': {'en': 'Suggested for You', 'sw': 'Mapendekezo Kwako'},
    'all_caught_up': {'en': 'All caught up!', 'sw': 'Umesoma yote!'},
    'nothing_here_yet': {
      'en': 'Nothing here yet',
      'sw': 'Hakuna chochote bado',
    },
    'from': {'en': 'From', 'sw': 'Kuanzia'},
    'today': {'en': 'Today', 'sw': 'Leo'},
    'view': {'en': 'View', 'sw': 'Angalia'},
    'view_all': {'en': 'View All', 'sw': 'Angalia Zote'},
    'view_details': {'en': 'View Details', 'sw': 'Angalia Maelezo'},
    'add': {'en': 'Add', 'sw': 'Ongeza'},
    'remove': {'en': 'Remove', 'sw': 'Ondoa'},
    'apply': {'en': 'Apply', 'sw': 'Tumia'},
    'filter': {'en': 'Filter', 'sw': 'Chuja'},
    'sort': {'en': 'Sort', 'sw': 'Panga'},
    'back': {'en': 'Back', 'sw': 'Rudi'},
    'yes': {'en': 'Yes', 'sw': 'Ndio'},
    'no': {'en': 'No', 'sw': 'Hapana'},
    'ok': {'en': 'OK', 'sw': 'Sawa'},
    'loading': {'en': 'Loading...', 'sw': 'Inapakia...'},
    'no_results': {'en': 'No results found', 'sw': 'Hakuna matokeo'},
    'error': {'en': 'Error', 'sw': 'Hitilafu'},
    'success': {'en': 'Success', 'sw': 'Imefanikiwa'},
    'warning': {'en': 'Warning', 'sw': 'Onyo'},
    'info': {'en': 'Info', 'sw': 'Taarifa'},
    'save_changes': {'en': 'Save Changes', 'sw': 'Hifadhi Mabadiliko'},

    // ── Moments / Social ──
    'create_post': {'en': 'Create Post', 'sw': 'Andika Chapisho'},
    'whats_on_your_mind': {
      'en': "What's on your mind?",
      'sw': 'Una nini akilini?',
    },
    'like': {'en': 'Like', 'sw': 'Penda'},
    'comment': {'en': 'Comment', 'sw': 'Maoni'},
    'comments': {'en': 'Comments', 'sw': 'Maoni'},
    'write_comment': {'en': 'Write a comment...', 'sw': 'Andika maoni...'},
    'reply': {'en': 'Reply', 'sw': 'Jibu'},
    'echoes': {'en': 'Echoes', 'sw': 'Echoes'},
    'echo': {'en': 'Echo', 'sw': 'Echo'},
    'glows': {'en': 'Glows', 'sw': 'Glows'},
    'glow': {'en': 'Glow', 'sw': 'Glow'},

    // ── Communities ──
    'communities': {'en': 'Communities', 'sw': 'Jumuiya'},
    'join': {'en': 'Join', 'sw': 'Jiunge'},
    'leave': {'en': 'Leave', 'sw': 'Ondoka'},
    'members': {'en': 'Members', 'sw': 'Wanachama'},
    'admin': {'en': 'Admin', 'sw': 'Admin'},

    // ── Invitations ──
    'invitations': {'en': 'Invitations', 'sw': 'Mialiko'},
    'invitation_card': {'en': 'Invitation Card', 'sw': 'Kadi ya Mwaliko'},
    'send_invitation': {'en': 'Send Invitation', 'sw': 'Tuma Mwaliko'},
    'invited_events': {'en': 'Invited Events', 'sw': 'Matukio Uliyoalikwa'},

    // ── Reports ──
    'reports': {'en': 'Reports', 'sw': 'Ripoti'},
    'generate_report': {'en': 'Generate Report', 'sw': 'Tengeneza Ripoti'},
    'download_report': {'en': 'Download Report', 'sw': 'Pakua Ripoti'},
    'export': {'en': 'Export', 'sw': 'Hamisha'},
    'import': {'en': 'Import', 'sw': 'Ingiza'},

    // ── Budget ──
    'budget_assistant': {'en': 'Budget Assistant', 'sw': 'Msaidizi wa Bajeti'},
    'total_budget': {'en': 'Total Budget', 'sw': 'Bajeti Jumla'},
    'spent': {'en': 'Spent', 'sw': 'Imetumika'},
    'remaining': {'en': 'Remaining', 'sw': 'Imebaki'},
    'income': {'en': 'Income', 'sw': 'Mapato'},
    'expenses': {'en': 'Expenses', 'sw': 'Matumizi'},

    // ── Photos ──
    'photos': {'en': 'Photos', 'sw': 'Picha'},
    'photo_library': {'en': 'Photo Library', 'sw': 'Hifadhi ya Picha'},
    'upload_photo': {'en': 'Upload Photo', 'sw': 'Pakia Picha'},
    'take_photo': {'en': 'Take Photo', 'sw': 'Piga Picha'},
    'choose_from_gallery': {
      'en': 'Choose from Gallery',
      'sw': 'Chagua kutoka Gallery',
    },
    'change_profile_photo': {
      'en': 'Change Profile Photo',
      'sw': 'Badilisha Picha ya Wasifu',
    },
    'use_camera': {'en': 'Use your camera', 'sw': 'Tumia kamera yako'},
    'pick_from_photos': {
      'en': 'Pick from your photos',
      'sw': 'Chagua kutoka picha zako',
    },
    'crop_profile_photo': {
      'en': 'Crop Profile Photo',
      'sw': 'Kata Picha ya Wasifu',
    },

    // ── QR / Scanning ──
    'scan_qr': {'en': 'Scan QR Code', 'sw': 'Scan QR Code'},
    'qr_code': {'en': 'QR Code', 'sw': 'QR Code'},

    // ── Help ──
    'help': {'en': 'Help', 'sw': 'Msaada'},
    'help_center': {'en': 'Help Center', 'sw': 'Kituo cha Msaada'},
    'contact_us': {'en': 'Contact Us', 'sw': 'Wasiliana Nasi'},
    'faq': {'en': 'FAQ', 'sw': 'Maswali Yanayoulizwa Mara kwa Mara'},

    // ── Issues ──
    'issues': {'en': 'Issues', 'sw': 'Matatizo'},
    'report_issue': {'en': 'Report Issue', 'sw': 'Ripoti Tatizo'},
    'my_issues': {'en': 'My Issues', 'sw': 'Matatizo Yangu'},

    // ── Saved ──
    'saved': {'en': 'Saved', 'sw': 'Zilizohifadhiwa'},
    'saved_posts': {'en': 'Saved Posts', 'sw': 'Machapisho Yaliyohifadhiwa'},

    // ── Circle ──
    'circle': {'en': 'Circle', 'sw': 'Duara'},
    'my_circle': {'en': 'My Circle', 'sw': 'Duara Langu'},

    // ── Misc ──
    'yesterday': {'en': 'Yesterday', 'sw': 'Jana'},
    'tomorrow': {'en': 'Tomorrow', 'sw': 'Kesho'},
    'see_more': {'en': 'See More', 'sw': 'Angalia Zaidi'},
    'see_less': {'en': 'See Less', 'sw': 'Angalia Kidogo'},
    'no_events': {'en': 'No events yet', 'sw': 'Hakuna matukio bado'},
    'no_moments': {'en': 'No moments yet', 'sw': 'Hakuna moments bado'},
    'no_moments_yet': {'en': 'No moments yet', 'sw': 'Hakuna moments bado'},
    'share_first_moment': {
      'en': 'Share your first moment!',
      'sw': 'Shiriki moment yako ya kwanza!',
    },
    'no_notifications': {'en': 'No notifications', 'sw': 'Hakuna arifa'},
    'no_messages': {'en': 'No messages', 'sw': 'Hakuna ujumbe'},
    'empty_state': {'en': 'Nothing here yet', 'sw': 'Hakuna kitu bado'},
    'pull_to_refresh': {'en': 'Pull to refresh', 'sw': 'Vuta ili kusasisha'},
    'are_you_sure': {'en': 'Are you sure?', 'sw': 'Una uhakika?'},
    'this_action_cannot_be_undone': {
      'en': 'This action cannot be undone.',
      'sw': 'Kitendo hiki hakiwezi kurudishwa.',
    },
    'profile_updated': {
      'en': 'Profile updated successfully',
      'sw': 'Wasifu umesasishwa',
    },
    'no_changes': {
      'en': 'No changes to save',
      'sw': 'Hakuna mabadiliko ya kuhifadhi',
    },
    'required_field': {
      'en': 'This field cannot be empty',
      'sw': 'Sehemu hii haiwezi kuwa tupu',
    },
    'invalid_email': {
      'en': 'Enter a valid email address',
      'sw': 'Weka anwani sahihi ya email',
    },
    'password_too_short': {
      'en': 'Password is too short',
      'sw': 'Password ni fupi sana',
    },
    'verify_phone': {
      'en': 'Verify Phone Number',
      'sw': 'Thibitisha Namba ya Simu',
    },

    // ── Navigation ──
    'nav_home': {'en': 'Home', 'sw': 'Nyumbani'},
    'nav_events': {'en': 'Events', 'sw': 'Matukio'},
    'nav_create': {'en': 'Create', 'sw': 'Unda'},
    'nav_services': {'en': 'Services', 'sw': 'Huduma'},
    'nav_profile': {'en': 'Profile', 'sw': 'Wasifu'},

    // ── Invited ──
    'invited': {'en': 'Invited', 'sw': 'Umealikwa'},
    'accept': {'en': 'Accept', 'sw': 'Kubali'},
    'decline': {'en': 'Decline', 'sw': 'Kataa'},

    // ── Status messages ──
    'connection_error': {
      'en': 'Please check your connection and try again.',
      'sw': 'Tafadhali angalia muunganisho wako na ujaribu tena.',
    },
    'session_expired': {
      'en': 'Session expired. Please sign in again.',
      'sw': 'Muda wa kikao umeisha. Tafadhali ingia tena.',
    },
    'feature_coming_soon': {
      'en': 'Feature coming soon',
      'sw': 'Kipengele kinakuja hivi karibuni',
    },
    'update_available': {
      'en': 'Update available',
      'sw': 'Sasisho linapatikana',
    },

    // ── Signup flow ──
    'whats_your_name': {'en': "What's your name?", 'sw': 'Jina lako ni nani?'},
    'lets_start': {
      'en': "Let's start with the basics",
      'sw': 'Tuanze na mambo ya msingi',
    },
    'your_first_name': {'en': 'Your first name', 'sw': 'Jina lako la kwanza'},
    'your_last_name': {'en': 'Your last name', 'sw': 'Jina lako la mwisho'},
    'looks_good': {'en': 'Looks good', 'sw': 'Inaonekana vizuri'},
    'choose_username': {'en': 'Choose a username', 'sw': 'Chagua username'},
    'how_others_find_you': {
      'en': 'This is how others will find you',
      'sw': 'Hivi ndivyo wengine watakupataje',
    },
    'checking': {'en': 'Checking...', 'sw': 'Inakagua...'},
    'available_text': {'en': 'Available', 'sw': 'Inapatikana'},
    'taken': {'en': 'Taken', 'sw': 'Imechukuliwa'},
    'try_one_of_these': {'en': 'Try one of these:', 'sw': 'Jaribu mojawapo:'},
    'secure_account': {'en': 'Secure your account', 'sw': 'Linda akaunti yako'},
    'create_strong_password': {
      'en': 'Create a strong password',
      'sw': 'Tengeneza password imara',
    },
    'create_password': {'en': 'Create a password', 'sw': 'Tengeneza password'},
    'reenter_password': {
      'en': 'Re-enter your password',
      'sw': 'Ingiza tena password yako',
    },
    'verify_your_phone': {
      'en': 'Verify your phone',
      'sw': 'Thibitisha simu yako',
    },
    'well_send_code': {
      'en': "We'll send you a verification code",
      'sw': 'Tutakutumia nambari ya uthibitisho',
    },
    'enter_verification_code': {
      'en': 'Enter verification code',
      'sw': 'Ingiza nambari ya uthibitisho',
    },
    'verify_and_continue': {
      'en': 'Verify & Continue',
      'sw': 'Thibitisha na Endelea',
    },
    'sending': {'en': 'Sending...', 'sw': 'Inatuma...'},
    'didnt_get_code': {
      'en': "Didn't get a code? Resend",
      'sw': 'Hukupata nambari? Tuma tena',
    },
    'check_whatsapp': {'en': 'Check WhatsApp', 'sw': 'Angalia WhatsApp'},
    'check_sms': {'en': 'Check SMS', 'sw': 'Angalia SMS'},
    'check_whatsapp_and_sms': {
      'en': 'Check WhatsApp and SMS',
      'sw': 'Angalia WhatsApp na SMS',
    },
    'welcome_user': {'en': 'Welcome', 'sw': 'Karibu'},
    'account_verified': {
      'en': 'Your account is verified.\nTaking you to your workspace...',
      'sw':
          'Akaunti yako imethibitishwa.\nTunakupeleka kwenye workspace yako...',
    },

    // ── Forgot Password ──
    'reset_via_email': {
      'en': 'Reset via email',
      'sw': 'Weka upya kupitia email',
    },
    'reset_via_phone': {
      'en': 'Reset via phone',
      'sw': 'Weka upya kupitia simu',
    },
    'enter_code': {'en': 'Enter code', 'sw': 'Ingiza nambari'},
    'choose_recovery': {
      'en': 'Choose how to recover your account',
      'sw': 'Chagua jinsi ya kurejesha akaunti yako',
    },
    'send_reset_link': {
      'en': 'Send reset link',
      'sw': 'Tuma kiungo cha kuweka upya',
    },
    'send_reset_code': {
      'en': 'Send reset code',
      'sw': 'Tuma nambari ya kuweka upya',
    },
    'well_send_reset_link': {
      'en': "We'll send a reset link",
      'sw': 'Tutatuma kiungo cha kuweka upya',
    },
    'well_send_otp': {
      'en': "We'll send an OTP code",
      'sw': 'Tutatuma nambari ya OTP',
    },
    'email_address': {'en': 'Email address', 'sw': 'Anwani ya email'},
    'enter_email_reset': {
      'en': 'Enter your email to receive reset instructions',
      'sw': 'Ingiza email yako kupokea maelekezo ya kuweka upya',
    },
    'enter_phone_reset': {
      'en': 'Enter your phone to receive a reset code',
      'sw': 'Ingiza simu yako kupokea nambari ya kuweka upya',
    },
    'choose_new_password': {
      'en': 'Choose a new secure password',
      'sw': 'Chagua password mpya salama',
    },
    'create_new_password': {
      'en': 'Create a new password',
      'sw': 'Tengeneza password mpya',
    },

    // ── Search ──
    'search_people': {'en': 'People', 'sw': 'Watu'},
    'search_nuru': {'en': 'Search Nuru...', 'sw': 'Tafuta Nuru...'},
    'no_users_found': {'en': 'No users found', 'sw': 'Hakuna watumiaji'},
    'no_events_found': {'en': 'No events found', 'sw': 'Hakuna matukio'},
    'no_services_found': {'en': 'No services found', 'sw': 'Hakuna huduma'},
    'search_hint': {
      'en': 'Search for people, events, or services',
      'sw': 'Tafuta watu, matukio, au huduma',
    },

    // ── Help ──
    'getting_started': {'en': 'Getting Started', 'sw': 'Kuanza'},
    'event_management': {
      'en': 'Event Management',
      'sw': 'Usimamizi wa Matukio',
    },
    'account_settings': {
      'en': 'Account Settings',
      'sw': 'Mipangilio ya Akaunti',
    },
    'safety_privacy': {'en': 'Safety & Privacy', 'sw': 'Usalama na Faragha'},
    'ai_assistant': {'en': 'AI Assistant', 'sw': 'Msaidizi wa AI'},
    'live_chat': {'en': 'Live Chat', 'sw': 'Chat ya Moja kwa Moja'},

    // ── Services ──
    'add_service': {'en': 'Add Service', 'sw': 'Ongeza Huduma'},
    'edit_service': {'en': 'Edit Service', 'sw': 'Hariri Huduma'},
    'service_details': {'en': 'Service Details', 'sw': 'Maelezo ya Huduma'},
    'manage_photos': {'en': 'Manage Photos', 'sw': 'Simamia Picha'},
    'manage_intro_clip': {
      'en': 'Manage Intro Clip',
      'sw': 'Simamia Video ya Utangulizi',
    },
    'service_verification': {
      'en': 'Service Verification',
      'sw': 'Uthibitisho wa Huduma',
    },
    'category': {'en': 'Category', 'sw': 'Kategoria'},
    'price_range': {'en': 'Price Range', 'sw': 'Bei'},
    'reviews': {'en': 'Reviews', 'sw': 'Tathmini'},
    'rating': {'en': 'Rating', 'sw': 'Ukadiriaji'},
    'book_now': {'en': 'Book Now', 'sw': 'Weka Sasa'},
    'contact_provider': {
      'en': 'Contact Vendor',
      'sw': 'Wasiliana na Mtoa Huduma',
    },
    'verified': {'en': 'Verified', 'sw': 'Imethibitishwa'},
    'unverified': {'en': 'Unverified', 'sw': 'Haijathibitishwa'},

    // ── Communities extra ──
    'create_community': {'en': 'Create Community', 'sw': 'Unda Jumuiya'},
    'community_details': {
      'en': 'Community Details',
      'sw': 'Maelezo ya Jumuiya',
    },
    'community_members': {'en': 'Members', 'sw': 'Wanachama'},
    'join_community': {'en': 'Join Community', 'sw': 'Jiunge na Jumuiya'},
    'leave_community': {'en': 'Leave Community', 'sw': 'Ondoka Jumuiya'},

    // ── Tickets extra ──
    'browse_tickets': {'en': 'Browse Tickets', 'sw': 'Tafuta Tiketi'},
    'ticket_details': {'en': 'Ticket Details', 'sw': 'Maelezo ya Tiketi'},
    'purchase_ticket': {'en': 'Purchase Ticket', 'sw': 'Nunua Tiketi'},
    'ticket_type': {'en': 'Ticket Type', 'sw': 'Aina ya Tiketi'},
    'quantity': {'en': 'Quantity', 'sw': 'Idadi'},
    'total': {'en': 'Total', 'sw': 'Jumla'},
    'checkout': {'en': 'Checkout', 'sw': 'Lipia'},

    // ── Event detail ──
    'event_overview': {'en': 'Overview', 'sw': 'Muhtasari'},
    'timeline': {'en': 'Timeline', 'sw': 'Ratiba'},
    'committee': {'en': 'Committee', 'sw': 'Kamati'},
    'agenda': {'en': 'Agenda', 'sw': 'Ajenda'},
    'announcements': {'en': 'Announcements', 'sw': 'Matangazo'},
    'share_event': {'en': 'Share Event', 'sw': 'Shiriki Tukio'},
    'manage_event': {'en': 'View Event', 'sw': 'Angalia Tukio'},
    'event_settings': {'en': 'Event Settings', 'sw': 'Mipangilio ya Tukio'},

    // ── Create Event ──
    'basic_info': {'en': 'Basic Info', 'sw': 'Taarifa za Msingi'},
    'date_and_time': {'en': 'Date & Time', 'sw': 'Tarehe na Wakati'},
    'location_venue': {'en': 'Location & Venue', 'sw': 'Mahali na Ukumbi'},
    'additional_details': {'en': 'Additional Details', 'sw': 'Maelezo Zaidi'},
    'publish_event': {'en': 'Publish Event', 'sw': 'Chapisha Tukio'},
    'save_draft': {'en': 'Save Draft', 'sw': 'Hifadhi Rasimu'},
    'select_date': {'en': 'Select Date', 'sw': 'Chagua Tarehe'},
    'select_time': {'en': 'Select Time', 'sw': 'Chagua Wakati'},
    'pick_location': {'en': 'Pick Location', 'sw': 'Chagua Mahali'},

    // ── Budget Assistant ──
    'add_item': {'en': 'Add Item', 'sw': 'Ongeza Kipengele'},
    'item_name': {'en': 'Item Name', 'sw': 'Jina la Kipengele'},
    'estimated_cost': {'en': 'Estimated Cost', 'sw': 'Gharama Inayokadiriwa'},
    'actual_cost': {'en': 'Actual Cost', 'sw': 'Gharama Halisi'},
    'paid': {'en': 'Paid', 'sw': 'Imelipwa'},
    'unpaid': {'en': 'Unpaid', 'sw': 'Haijalipwa'},

    // ── Reports ──
    'event_report': {'en': 'Event Report', 'sw': 'Ripoti ya Tukio'},
    'guest_report': {'en': 'Guest Report', 'sw': 'Ripoti ya Wageni'},
    'contribution_report': {
      'en': 'Contribution Report',
      'sw': 'Ripoti ya Michango',
    },
    'budget_report': {'en': 'Budget Report', 'sw': 'Ripoti ya Bajeti'},
    'preview_report': {'en': 'Preview Report', 'sw': 'Hakiki Ripoti'},

    // ── Photos extra ──
    'event_photos': {'en': 'Event Photos', 'sw': 'Picha za Tukio'},
    'shared_photos': {'en': 'Shared Photos', 'sw': 'Picha Zilizoshirikiwa'},
    'upload_photos': {'en': 'Upload Photos', 'sw': 'Pakia Picha'},

    // ── Invitation extra ──
    'send_invitations': {'en': 'Send Invitations', 'sw': 'Tuma Mialiko'},
    'invitation_qr': {'en': 'Invitation QR', 'sw': 'QR ya Mwaliko'},
    'download_card': {'en': 'Download Card', 'sw': 'Pakua Kadi'},

    // ── Map ──
    'map_picker': {'en': 'Pick Location', 'sw': 'Chagua Mahali'},
    'confirm_location': {'en': 'Confirm Location', 'sw': 'Thibitisha Mahali'},

    // ── Removed Content ──
    'removed_content': {
      'en': 'Removed Content',
      'sw': 'Maudhui Yaliyoondolewa',
    },
    'post_deleted': {'en': 'Post deleted', 'sw': 'Chapisho limefutwa'},

    // ── Moments ──
    'my_moments': {'en': 'My Moments', 'sw': 'Moments Zangu'},
    'no_posts_yet': {'en': 'No posts yet', 'sw': 'Hakuna machapisho bado'},
    'start_sharing': {
      'en': 'Share your first moment!',
      'sw': 'Shiriki moment yako ya kwanza!',
    },

    // ── Messages ──
    'new_message': {'en': 'New Message', 'sw': 'Ujumbe Mpya'},
    'type_message': {'en': 'Type a message...', 'sw': 'Andika ujumbe...'},
    'no_conversations': {
      'en': 'No conversations yet',
      'sw': 'Hakuna mazungumzo bado',
    },
    'search_conversations': {
      'en': 'Search conversations...',
      'sw': 'Tafuta mazungumzo...',
    },
    'online': {'en': 'Online', 'sw': 'Mtandaoni'},
    'offline': {'en': 'Offline', 'sw': 'Nje ya mtandao'},
    'typing': {'en': 'typing...', 'sw': 'anaandika...'},
    'delete_conversation': {
      'en': 'Delete conversation',
      'sw': 'Futa mazungumzo',
    },
    'delete_conversation_subtitle': {
      'en': 'Removes this chat from your list only.',
      'sw': 'Inaondoa mazungumzo haya kwenye orodha yako tu.',
    },
    'delete_conversation_confirm': {
      'en': 'Remove your conversation with {name}? It will only be hidden on your side.',
      'sw': 'Ondoa mazungumzo yako na {name}? Yatafichwa upande wako tu.',
    },

    // ── Splash ──
    'splash_tagline': {
      'en': 'Keep your event together',
      'sw': 'Weka tukio lako pamoja',
    },

    // ── Forgot Password extra ──
    'new_password_label': {'en': 'New Password', 'sw': 'Password Mpya'},
    'confirm_password_label': {
      'en': 'Confirm Password',
      'sw': 'Thibitisha Password',
    },
    'create_new_password_hint': {
      'en': 'Create a new password',
      'sw': 'Tengeneza password mpya',
    },
    'reenter_password_hint': {
      'en': 'Re-enter your password',
      'sw': 'Ingiza tena password yako',
    },
    'min_8_chars': {'en': 'At least 8 characters', 'sw': 'Angalau herufi 8'},
    'passwords_dont_match': {
      'en': "Passwords don't match",
      'sw': 'Password hazilingani',
    },
    'password_reset_success': {
      'en': 'Password reset! Please sign in.',
      'sw': 'Password imewekwa upya! Tafadhali ingia.',
    },
    'enter_your_email': {'en': 'Enter your email', 'sw': 'Ingiza email yako'},
    'enter_your_phone': {
      'en': 'Enter your phone',
      'sw': 'Ingiza namba yako ya simu',
    },
    'enter_6_digit_code': {
      'en': 'Enter the 6-digit code',
      'sw': 'Ingiza nambari ya tarakimu 6',
    },
    'reset_link_sent': {
      'en': 'Reset link sent!',
      'sw': 'Kiungo cha kuweka upya kimetumwa!',
    },
    'code_sent': {'en': 'Code sent', 'sw': 'Nambari imetumwa'},
    'verification_failed': {
      'en': 'Verification failed',
      'sw': 'Uthibitisho umeshindwa',
    },
    'reset_failed': {
      'en': "We couldn't reset your password. Please try again.",
      'sw': 'Hatukuweza kuweka upya password yako. Tafadhali jaribu tena.',
    },
    'we_sent_code_to': {
      'en': 'We sent a 6-digit code to',
      'sw': 'Tumetuma nambari ya tarakimu 6 kwa',
    },
    'email_hint': {
      'en': 'your.email@example.com',
      'sw': 'email.yako@mfano.com',
    },

    // ── Messages extra ──
    'new_conversation': {'en': 'New Conversation', 'sw': 'Mazungumzo Mapya'},
    'search_for_person': {
      'en': 'Search for a person to chat with',
      'sw': 'Tafuta mtu wa kuzungumza naye',
    },
    'start_conversation': {
      'en': 'Start a conversation with someone',
      'sw': 'Anza mazungumzo na mtu',
    },
    'say_hello': {'en': 'Say hello!', 'sw': 'Sema habari!'},
    'start_conv_with': {
      'en': 'Start your conversation with',
      'sw': 'Anza mazungumzo na',
    },
    'say_hello_to': {'en': 'Say hello to', 'sw': 'Sema habari kwa'},
    'write_message_to': {'en': 'Write a message to', 'sw': 'Andika ujumbe kwa'},
    'start_conversation_btn': {
      'en': 'Start Conversation',
      'sw': 'Anza Mazungumzo',
    },
    'sending_text': {'en': 'Sending...', 'sw': 'Inatuma...'},
    'no_messages_yet': {'en': 'No messages yet', 'sw': 'Hakuna ujumbe bado'},
    'photo': {'en': 'Photo', 'sw': 'Picha'},
    'chat': {'en': 'Chat', 'sw': 'Chat'},

    // ── Settings extra ──
    'nuru_pass': {'en': 'Nuru Pass', 'sw': 'Nuru Pass'},
    'personal_info': {'en': 'Personal Information', 'sw': 'Taarifa Binafsi'},
    'contact_info': {'en': 'Contact', 'sw': 'Mawasiliano'},
    'write_bio': {
      'en': 'Tell us about yourself...',
      'sw': 'Tuambie kuhusu wewe...',
    },
    'city_country': {'en': 'City, Country', 'sw': 'Jiji, Nchi'},
    'current_password': {'en': 'Current Password', 'sw': 'Password ya Sasa'},
    'enter_current_password': {
      'en': 'Enter current password',
      'sw': 'Ingiza password ya sasa',
    },
    'enter_new_password': {
      'en': 'Enter new password',
      'sw': 'Ingiza password mpya',
    },
    'confirm_new_password': {
      'en': 'Confirm new password',
      'sw': 'Thibitisha password mpya',
    },
    'password_changed': {
      'en': 'Password changed successfully',
      'sw': 'Password imebadilishwa',
    },
    'password_hint': {
      'en': 'Make sure your new password is at least 8 characters long.',
      'sw': 'Hakikisha password yako mpya ina angalau herufi 8.',
    },

    // ── Privacy & Security ──
    'private_profile': {'en': 'Private Profile', 'sw': 'Wasifu wa Faragha'},
    'private_profile_desc': {
      'en': 'Only approved followers can see your content',
      'sw': 'Wafuasi walioidhinishwa tu ndio wanaweza kuona maudhui yako',
    },
    'show_online_status': {
      'en': 'Show Online Status',
      'sw': 'Onyesha Hali ya Mtandaoni',
    },
    'show_online_status_desc': {
      'en': "Show when you're online",
      'sw': 'Onyesha wakati uko mtandaoni',
    },
    'show_last_seen': {'en': 'Show Last Seen', 'sw': 'Onyesha Mara ya Mwisho'},
    'show_last_seen_desc': {
      'en': 'Let others see when you were last active',
      'sw': 'Waache wengine waone ulipokuwa active mara ya mwisho',
    },
    'read_receipts': {'en': 'Read Receipts', 'sw': 'Risiti za Kusoma'},
    'read_receipts_desc': {
      'en': "Show when you've read messages",
      'sw': 'Onyesha wakati umesoma ujumbe',
    },
    'two_factor_auth': {
      'en': 'Two-Factor Authentication',
      'sw': 'Uthibitisho wa Hatua Mbili',
    },
    'enabled': {'en': 'Enabled', 'sw': 'Imewashwa'},
    'disabled': {'en': 'Disabled', 'sw': 'Imezimwa'},

    // ── Notifications settings ──
    'email_notifications': {
      'en': 'Email Notifications',
      'sw': 'Arifa za Email',
    },
    'email_notifications_desc': {
      'en': 'Receive email about your account activity',
      'sw': 'Pokea email kuhusu shughuli za akaunti yako',
    },
    'push_notifications': {'en': 'Push Notifications', 'sw': 'Arifa za Push'},
    'push_notifications_desc': {
      'en': 'Enable push notifications',
      'sw': 'Washa arifa za push',
    },
    'sms_notifications': {'en': 'SMS Notifications', 'sw': 'Arifa za SMS'},
    'sms_notifications_desc': {
      'en': 'Receive SMS for important updates',
      'sw': 'Pokea SMS kwa sasisho muhimu',
    },

    // ── About ──
    'open_source_licenses': {
      'en': 'Open Source Licenses',
      'sw': 'Leseni za Open Source',
    },
    'settings_updated': {
      'en': 'Settings updated',
      'sw': 'Mipangilio imesasishwa',
    },

    // ── Create Event extra ──
    'give_event_name': {
      'en': 'Give your event a name',
      'sw': 'Weka jina la tukio lako',
    },
    'event_venue_address': {
      'en': 'Event venue or address',
      'sw': 'Mahali pa tukio au anwani',
    },
    'venue_name': {'en': 'Venue Name', 'sw': 'Jina la Ukumbi'},
    'venue_name_optional': {
      'en': 'Venue name (optional)',
      'sw': 'Jina la ukumbi (hiari)',
    },
    'describe_event': {'en': 'Describe your event', 'sw': 'Eleza tukio lako'},
    'location_pinned': {
      'en': 'Location pinned on map',
      'sw': 'Mahali pamewekwa kwenye ramani',
    },
    'expected_guests': {'en': 'Expected Guests', 'sw': 'Wageni Wanaotarajiwa'},
    'estimated_budget': {
      'en': 'Estimated Budget (TZS)',
      'sw': 'Bajeti Inayokadiriwa (TZS)',
    },
    'cover_image': {'en': 'Cover Image', 'sw': 'Picha ya Jalada'},
    'tap_to_upload': {'en': 'Tap to upload image', 'sw': 'Gusa kupakia picha'},
    'visibility': {'en': 'Visibility', 'sw': 'Mwonekano'},
    'enable_ticketing': {'en': 'Enable Ticketing', 'sw': 'Washa Tiketi'},
    'enable_ticketing_desc': {
      'en': 'Allow selling tickets for this event',
      'sw': 'Ruhusu kuuza tiketi kwa tukio hili',
    },
    'dress_code_hint': {
      'en': 'Dress code (e.g., Black Tie)',
      'sw': 'Dress code (mfano, Black Tie)',
    },
    'special_instructions_hint': {
      'en': 'Special instructions for guests',
      'sw': 'Maelekezo maalum kwa wageni',
    },
    'end_date_optional': {
      'en': 'End Date (optional)',
      'sw': 'Tarehe ya Mwisho (hiari)',
    },
    'private': {'en': 'Private', 'sw': 'Binafsi'},
    'public': {'en': 'Public', 'sw': 'Hadharani'},

    // ── Moments extra ──
    'delete_moment': {'en': 'Delete Moment', 'sw': 'Futa Moment'},
    'edit_moment': {'en': 'Edit Moment', 'sw': 'Hariri Moment'},
    'moment_deleted': {'en': 'Moment deleted', 'sw': 'Moment imefutwa'},
    'moment_updated': {'en': 'Moment updated', 'sw': 'Moment imesasishwa'},
    'make_public': {'en': 'Make Public', 'sw': 'Fanya Hadharani'},
    'circle_only': {'en': 'Circle Only', 'sw': 'Duara Tu'},

    // ── Circle extra ──
    'no_members_circle': {
      'en': 'No members in your circle',
      'sw': 'Hakuna wanachama kwenye circle yako',
    },
    'no_pending_requests': {
      'en': 'No pending requests',
      'sw': 'Hakuna maombi yanasubiri',
    },
    'requests': {'en': 'Requests', 'sw': 'Maombi'},
    'unknown': {'en': 'Unknown', 'sw': 'Haijulikani'},

    // ── Communities extra ──
    'no_communities_found': {
      'en': 'No communities found',
      'sw': 'Hakuna jumuiya zilizopatikana',
    },
    'not_joined_communities': {
      'en': "You haven't joined any communities",
      'sw': 'Hujajiunga na jumuiya yoyote',
    },
    'members_count': {'en': 'members', 'sw': 'wanachama'},
    'create': {'en': 'Create', 'sw': 'Unda'},
    'be_first_post': {
      'en': 'Be the first to post in this community',
      'sw': 'Kuwa wa kwanza kuchapisha kwenye jumuiya hii',
    },
    'no_members': {'en': 'No members', 'sw': 'Hakuna wanachama'},
    'member': {'en': 'Member', 'sw': 'Mwanachama'},

    // ── Contributors ──
    'delete_contributor': {'en': 'Delete Contributor', 'sw': 'Futa Mchangiaji'},
    'confirm_remove_contributor': {
      'en': 'Are you sure you want to remove this contributor?',
      'sw': 'Una uhakika unataka kuondoa mchangiaji huyu?',
    },
    'contributor_removed': {
      'en': 'Contributor removed',
      'sw': 'Mchangiaji ameondolewa',
    },
    'failed_delete': {
      'en': "We couldn't delete that. Please try again.",
      'sw': 'Hatukuweza kufuta. Tafadhali jaribu tena.',
    },
    'edit_contributor': {'en': 'Edit Contributor', 'sw': 'Hariri Mchangiaji'},
    'contributor_updated': {
      'en': 'Contributor updated',
      'sw': 'Mchangiaji amesasishwa',
    },
    'failed_update': {
      'en': "We couldn't save that change. Please try again.",
      'sw': 'Hatukuweza kuhifadhi mabadiliko. Tafadhali jaribu tena.',
    },
    'no_contributors_yet': {
      'en': 'No contributors yet',
      'sw': 'Hakuna wachangiaji bado',
    },
    'contributor_address_book': {
      'en': 'Your contributor address book will appear here',
      'sw': 'Kitabu chako cha anwani za wachangiaji kitaonekana hapa',
    },
    'name_field': {'en': 'Name', 'sw': 'Jina'},
    'phone_field': {'en': 'Phone', 'sw': 'Simu'},
    'email_field': {'en': 'Email', 'sw': 'Email'},
    'notes_field': {'en': 'Notes', 'sw': 'Maelezo'},

    // ── Budget Assistant extra ──
    'ai_budget_assistant': {
      'en': 'AI Budget Assistant',
      'sw': 'Msaidizi wa Bajeti wa AI',
    },
    'powered_by_nuru_ai': {
      'en': 'Powered by Nuru AI',
      'sw': 'Inaendeshwa na Nuru AI',
    },
    'thinking': {'en': 'Thinking...', 'sw': 'Inafikiri...'},
    'budget_estimate': {'en': 'Budget Estimate', 'sw': 'Makadirio ya Bajeti'},
    'generate_budget_first': {
      'en': 'Generate a budget estimate first',
      'sw': 'Tengeneza makadirio ya bajeti kwanza',
    },
    'failed_generate_pdf': {
      'en': "We couldn't generate the PDF. Please try again.",
      'sw': 'Hatukuweza kutengeneza PDF. Tafadhali jaribu tena.',
    },
    'preparing_pdf': {'en': 'Preparing PDF...', 'sw': 'Inaandaa PDF...'},
    'preview_pdf': {'en': 'Preview PDF', 'sw': 'Hakiki PDF'},
    'import_items': {'en': 'Import Items', 'sw': 'Ingiza Vipengele'},

    // ── Document verification ──
    'document_type': {'en': 'Document Type', 'sw': 'Aina ya Hati'},
    'upload_documents': {'en': 'Upload Documents', 'sw': 'Pakia Hati'},
    'submit_verification': {
      'en': 'Submit for Verification',
      'sw': 'Tuma kwa Uthibitisho',
    },

    // ── Auth extra ──
    'required': {'en': 'Required', 'sw': 'Inahitajika'},
    'use_real_name': {
      'en': 'Use your real name',
      'sw': 'Tumia jina lako halisi',
    },
    'enter_first_last_name': {
      'en': 'Please enter your first and last name',
      'sw': 'Tafadhali weka jina lako la kwanza na la mwisho',
    },
    'fix_name_errors': {
      'en': 'Please fix the name errors',
      'sw': 'Tafadhali rekebisha makosa ya jina',
    },
    'username_min_chars': {
      'en': 'Username must be at least 3 characters',
      'sw': 'Username lazima iwe angalau herufi 3',
    },
    'username_taken': {
      'en': 'Username is taken',
      'sw': 'Username imechukuliwa',
    },
    'meet_password_requirements': {
      'en': 'Meet all password requirements',
      'sw': 'Timiza mahitaji yote ya password',
    },
    'enter_valid_phone': {
      'en': 'Enter a valid phone number',
      'sw': 'Weka namba ya simu halali',
    },
    'signup_failed': {
      'en': "We couldn't create your account. Please try again.",
      'sw': 'Hatukuweza kuunda akaunti yako. Tafadhali jaribu tena.',
    },
    'account_created_verify': {
      'en': 'Account created! Verify your phone.',
      'sw': 'Akaunti imeundwa! Thibitisha simu yako.',
    },
    'verification_code_sent': {
      'en': 'Verification code sent',
      'sw': 'Msimbo wa uthibitisho umetumwa',
    },
    'code_sent_sms': {
      'en': 'Code sent via SMS',
      'sw': 'Msimbo umetumwa kwa SMS',
    },
    'failed_send_code': {
      'en': "We couldn't send the verification code. Please try again.",
      'sw': 'Hatukuweza kutuma msimbo wa uthibitisho. Tafadhali jaribu tena.',
    },
    'verified_signin_failed': {
      'en': 'Verified but sign-in failed. Please try again.',
      'sw': 'Imethibitishwa lakini kuingia kumeshindwa. Tafadhali jaribu tena.',
    },
    'invalid_otp': {
      'en': 'Invalid code. Please check and try again.',
      'sw': 'Msimbo si sahihi. Tafadhali angalia na ujaribu tena.',
    },
    'at_least_8_chars': {
      'en': 'At least 8 characters',
      'sw': 'Angalau herufi 8',
    },
    'one_number': {'en': 'One number', 'sw': 'Nambari moja'},
    'one_special_char': {
      'en': 'One special character',
      'sw': 'Alama moja maalum',
    },
    'continue': {'en': 'Continue', 'sw': 'Endelea'},
    'welcome_name': {'en': 'Welcome,', 'sw': 'Karibu,'},
    'welcome_back_msg': {'en': 'Welcome back!', 'sw': 'Karibu tena!'},
    'login_failed': {
      'en': "We couldn't sign you in. Please check your details and try again.",
      'sw':
          'Hatukuweza kukuingiza. Tafadhali angalia taarifa zako na ujaribu tena.',
    },
    'account_suspended': {
      'en': 'Your account has been suspended. Contact support for help.',
      'sw': 'Akaunti yako imesimamishwa. Wasiliana na msaada kwa usaidizi.',
    },

    // ── Event Detail Tabs ──
    'overview': {'en': 'Overview', 'sw': 'Muhtasari'},
    'checklist': {'en': 'Checklist', 'sw': 'Orodha'},
    'check_in': {'en': 'Check-In', 'sw': 'Check-In'},
    'schedule': {'en': 'Schedule', 'sw': 'Ratiba'},
    'contribution_summary': {
      'en': 'Contribution Summary',
      'sw': 'Muhtasari wa Michango',
    },
    'event_budget': {'en': 'Event Budget', 'sw': 'Bajeti ya Tukio'},
    'total_collected': {'en': 'Total Collected', 'sw': 'Jumla Iliyokusanywa'},
    'budget_shortfall': {'en': 'Budget Shortfall', 'sw': 'Upungufu wa Bajeti'},
    'total_pledged': {'en': 'Total Pledged', 'sw': 'Jumla ya Ahadi'},
    'outstanding': {'en': 'Outstanding', 'sw': 'Bado Hajalipwa'},
    'unpledged': {'en': 'Unpledged', 'sw': 'Bila Ahadi'},
    'budget_vs_raised': {
      'en': 'Budget vs Raised',
      'sw': 'Bajeti dhidi ya Kilichokusanywa',
    },
    'collection_progress': {
      'en': 'Collection Progress',
      'sw': 'Maendeleo ya Ukusanyaji',
    },
    'add_contributor': {'en': 'Add Contributor', 'sw': 'Ongeza Mchangiaji'},
    'bulk_upload': {'en': 'Bulk Upload', 'sw': 'Pakia Kwa Wingi'},
    'record_payment': {'en': 'Record Payment', 'sw': 'Rekodi Malipo'},
    'update_pledge': {'en': 'Update Pledge', 'sw': 'Sasisha Ahadi'},
    'payment_history': {'en': 'Payment History', 'sw': 'Historia ya Malipo'},
    'send_thank_you': {'en': 'Send Thank You', 'sw': 'Tuma Shukrani'},
    'add_as_guest': {'en': 'Add as Guest', 'sw': 'Ongeza kama Mgeni'},
    'remove_contributor': {
      'en': 'Remove Contributor',
      'sw': 'Ondoa Mchangiaji',
    },
    'financial_overview': {
      'en': 'Financial Overview',
      'sw': 'Muhtasari wa Fedha',
    },
    'budget_status': {'en': 'Budget Status', 'sw': 'Hali ya Bajeti'},
    'budget_allocated': {'en': 'Budget allocated', 'sw': 'Bajeti iliyotengwa'},
    'cash_in_hand': {'en': 'Cash in Hand', 'sw': 'Pesa Mkononi'},
    'paid_contributors': {
      'en': 'Paid contributors',
      'sw': 'Wachangiaji waliolipa',
    },
    'collection_rate': {
      'en': 'Collection rate',
      'sw': 'Kiwango cha ukusanyaji',
    },
    'event_progress': {'en': 'Event Progress', 'sw': 'Maendeleo ya Tukio'},
    'guest_overview': {'en': 'Guest Overview', 'sw': 'Muhtasari wa Wageni'},
    'confirmed_guests': {
      'en': 'Confirmed Guests',
      'sw': 'Wageni Waliothibitishwa',
    },
    'event_description': {'en': 'Event Description', 'sw': 'Maelezo ya Tukio'},
    'edit_event_btn': {'en': 'Edit Event', 'sw': 'Hariri Tukio'},
    'publish_event_btn': {'en': 'Publish Event', 'sw': 'Chapisha Tukio'},
    'cancel_event': {'en': 'Cancel Event', 'sw': 'Ghairi Tukio'},
    'delete_event_confirm': {'en': 'Delete Event?', 'sw': 'Futa Tukio?'},
    'action_cannot_undone': {
      'en': 'This action cannot be undone.',
      'sw': 'Kitendo hiki hakiwezi kurudishwa.',
    },
    'event_deleted': {'en': 'Event deleted', 'sw': 'Tukio limefutwa'},
    'event_published': {'en': 'Event published', 'sw': 'Tukio limechapishwa'},
    'event_cancelled': {'en': 'Event cancelled', 'sw': 'Tukio limeghairiwa'},
    'generating_report': {
      'en': 'Generating report...',
      'sw': 'Inaandaa ripoti...',
    },
    'report_saved': {'en': 'Report saved', 'sw': 'Ripoti imehifadhiwa'},
    'awaiting_confirmation': {
      'en': 'Awaiting Confirmation',
      'sw': 'Inasubiri Uthibitisho',
    },
    'select_all': {'en': 'Select All', 'sw': 'Chagua Zote'},
    'deselect_all': {'en': 'Deselect All', 'sw': 'Ondoa Zote'},
    'contributor_messaging': {
      'en': 'Contributor Messaging',
      'sw': 'Ujumbe kwa Wachangiaji',
    },
    'no_contribution': {'en': 'No Contribution', 'sw': 'Hakuna Mchango'},
    'partial_payment': {'en': 'Partial', 'sw': 'Sehemu'},
    'completed_payment': {'en': 'Completed', 'sw': 'Imekamilika'},
    'recipients': {'en': 'Recipients', 'sw': 'Wapokeaji'},
    'message_template': {'en': 'Message Template', 'sw': 'Kiolezo cha Ujumbe'},
    'reset': {'en': 'Reset', 'sw': 'Weka Upya'},
    'preview': {'en': 'Preview', 'sw': 'Hakiki'},
    'confirm_send': {'en': 'Confirm & Send', 'sw': 'Thibitisha na Tuma'},
    'send_results': {'en': 'Send Results', 'sw': 'Matokeo ya Kutuma'},
    'sent': {'en': 'Sent', 'sw': 'Imetumwa'},
    'failed': {'en': 'Failed', 'sw': 'Imeshindwa'},
    'new_contributor': {'en': 'New Contributor', 'sw': 'Mchangiaji Mpya'},
    'address_book': {'en': 'Address Book', 'sw': 'Kitabu cha Anwani'},
    'pledge_amount': {
      'en': 'Pledge Amount (TZS)',
      'sw': 'Kiasi cha Ahadi (TZS)',
    },
    'payment_method': {'en': 'Payment Method', 'sw': 'Njia ya Malipo'},
    'payment_reference': {
      'en': 'Payment Reference',
      'sw': 'Kumbukumbu ya Malipo',
    },
    'search_contributors': {
      'en': 'Search contributors...',
      'sw': 'Tafuta wachangiaji...',
    },
    'no_contributors_match': {
      'en': 'No contributors match your search.',
      'sw': 'Hakuna wachangiaji wanaolingana na utafutaji wako.',
    },
    'no_contributors_added': {
      'en':
          'No contributors added yet.\nAdd people and start tracking what comes in.',
      'sw':
          'Hakuna wachangiaji bado.\nOngeza watu na uanze kufuatilia michango.',
    },
    'pdf_report': {'en': 'PDF Report', 'sw': 'Ripoti ya PDF'},
    'excel_report': {'en': 'Excel (CSV)', 'sw': 'Excel (CSV)'},
    'bulk_upload_contributors': {
      'en': 'Bulk Upload Contributors',
      'sw': 'Pakia Wachangiaji kwa Wingi',
    },
    'upload_mode': {'en': 'Upload Mode', 'sw': 'Hali ya Kupakia'},
    'set_pledge_targets': {
      'en': 'Set Pledge Targets',
      'sw': 'Weka Malengo ya Ahadi',
    },
    'record_contributions': {
      'en': 'Record Contributions',
      'sw': 'Rekodi Michango',
    },
    'choose_file': {
      'en': 'Choose File (.xlsx, .csv)',
      'sw': 'Chagua Faili (.xlsx, .csv)',
    },
    'send_notifications': {'en': 'Send notifications', 'sw': 'Tuma arifa'},
    'upload_complete': {'en': 'Upload Complete', 'sw': 'Kupakia Kumekamilika'},

    // ── Event Detail extra ──
    'not_set': {'en': 'Not set', 'sw': 'Haijawekwa'},
    'pledged': {'en': 'Pledged', 'sw': 'Ahadi'},
    'of': {'en': 'of', 'sw': 'kati ya'},
    'guests_confirmed_attendance': {
      'en': 'Guests confirmed attendance',
      'sw': 'Wageni waliothibitisha kuhudhuria',
    },
    'event_photo_libraries': {
      'en': 'Event Photo Libraries',
      'sw': 'Maktaba za Picha za Tukio',
    },
    'event_summary_report': {
      'en': 'Event Summary Report',
      'sw': 'Ripoti ya Muhtasari wa Tukio',
    },
    'preview_and_share': {'en': 'Preview and share', 'sw': 'Hakiki na shiriki'},
    'open_in_spreadsheet': {
      'en': 'Open in spreadsheet apps',
      'sw': 'Fungua kwenye programu za karatasi',
    },

    // ── Meetings ──
    'meetings': {'en': 'Meetings', 'sw': 'Mikutano'},
    'meetings_subtitle': {
      'en': 'Coordinate with your team.',
      'sw': 'Ratibu na timu yako.',
    },
    'schedule_meeting': {'en': 'Schedule', 'sw': 'Panga'},
    'schedule_a_meeting': {'en': 'Schedule a meeting', 'sw': 'Panga mkutano'},
    'schedule_first_meeting': {
      'en': 'Schedule your first meeting',
      'sw': 'Panga mkutano wako wa kwanza',
    },
    'no_meetings_yet': {'en': 'No meetings yet', 'sw': 'Hakuna mikutano bado'},
    'no_meetings_desc': {
      'en':
          'Schedule a meeting to coordinate with your team. Everyone stays aligned.',
      'sw': 'Panga mkutano kuratibu na timu yako. Kila mtu anakuwa sawa.',
    },
    'meeting_title': {'en': 'Meeting title', 'sw': 'Jina la mkutano'},
    'meeting_title_placeholder': {
      'en': 'e.g. Budget review',
      'sw': 'mf. Mapitio ya bajeti',
    },
    'meeting_description': {
      'en': 'Description (optional)',
      'sw': 'Maelezo (si lazima)',
    },
    'meeting_description_placeholder': {
      'en': 'What will you discuss?',
      'sw': 'Mtajadili nini?',
    },
    'meeting_invite_subtitle': {
      'en': 'Bring your team together. Participants will be notified.',
      'sw': 'Kusanya timu yako. Washiriki wataarifiwa.',
    },
    'pick_date': {'en': 'Pick date', 'sw': 'Chagua tarehe'},
    'pick_time': {'en': 'Pick time', 'sw': 'Chagua saa'},
    'duration': {'en': 'Duration', 'sw': 'Muda'},
    'minutes_30': {'en': '30 minutes', 'sw': 'Dakika 30'},
    'hour_1': {'en': '1 hour', 'sw': 'Saa 1'},
    'hours_1_5': {'en': '1.5 hours', 'sw': 'Saa 1.5'},
    'hours_2': {'en': '2 hours', 'sw': 'Saa 2'},
    'schedule_meeting_btn': {'en': 'Schedule meeting', 'sw': 'Panga mkutano'},
    'enter_title_date_time': {
      'en': 'Enter a title, date, and time.',
      'sw': 'Weka jina, tarehe, na saa.',
    },
    'meeting_scheduled_invites': {
      'en': 'Meeting scheduled. Invitations sent.',
      'sw': 'Mkutano umepangwa. Mialiko imetumwa.',
    },
    'something_went_wrong': {
      'en': 'Something went wrong. Please try again.',
      'sw': 'Kuna tatizo. Tafadhali jaribu tena.',
    },
    'join_now': {'en': 'Join now', 'sw': 'Jiunge sasa'},
    'join_meeting': {'en': 'Join', 'sw': 'Jiunge'},
    'could_not_join': {
      'en': "We couldn't join the meeting. Please try again.",
      'sw': 'Hatukuweza kujiunga na mkutano. Tafadhali jaribu tena.',
    },
    'cancel_meeting': {'en': 'Cancel meeting', 'sw': 'Ghairi mkutano'},
    'cancel_meeting_q': {'en': 'Cancel meeting?', 'sw': 'Ghairi mkutano?'},
    'cancel_meeting_confirm': {
      'en':
          'This meeting will be removed and participants will no longer have access.',
      'sw': 'Mkutano huu utaondolewa na washiriki hawataweza kuufikia tena.',
    },
    'go_back': {'en': 'Go back', 'sw': 'Rudi nyuma'},
    'meeting_cancelled': {
      'en': 'Meeting cancelled.',
      'sw': 'Mkutano umeghairiwa.',
    },
    'copy_link': {'en': 'Copy link', 'sw': 'Nakili kiungo'},
    'status_scheduled': {'en': 'Scheduled', 'sw': 'Imepangwa'},
    'status_live': {'en': '● Live', 'sw': '● Moja kwa moja'},
    'status_ended': {'en': 'Ended', 'sw': 'Imemalizika'},
    'min_suffix': {'en': 'min', 'sw': 'dak'},
    'created_by': {'en': 'Created by', 'sw': 'Imeundwa na'},
    'edit_meeting': {'en': 'Edit', 'sw': 'Hariri'},
    'edit_a_meeting': {'en': 'Edit meeting', 'sw': 'Hariri mkutano'},
    'edit_meeting_subtitle': {
      'en': 'Update meeting details. Only scheduled meetings can be edited.',
      'sw':
          'Sasisha maelezo ya mkutano. Mikutano iliyopangwa tu ndiyo inayoweza kuhaririwa.',
    },
    'meeting_updated': {'en': 'Meeting updated.', 'sw': 'Mkutano umesasishwa.'},
    'update_meeting': {'en': 'Update Meeting', 'sw': 'Sasisha Mkutano'},

    // Meeting Agenda & Minutes
    'minutes': {'en': 'Minutes', 'sw': 'Muhtasari'},
    'agenda_minutes': {'en': 'Agenda & Minutes', 'sw': 'Ajenda na Muhtasari'},
    'no_agenda_yet': {'en': 'No agenda items yet', 'sw': 'Hakuna ajenda bado'},
    'no_agenda_desc': {
      'en': 'Add agenda items to structure your meeting.',
      'sw': 'Ongeza vipengele vya ajenda kupanga mkutano.',
    },
    'add_agenda_item': {
      'en': 'Add Agenda Item',
      'sw': 'Ongeza Kipengele cha Ajenda',
    },
    'agenda_title_placeholder': {
      'en': 'e.g. Review quarterly budget',
      'sw': 'k.m. Kagua bajeti ya robo mwaka',
    },
    'agenda_desc_placeholder': {
      'en': 'Additional details...',
      'sw': 'Maelezo zaidi...',
    },
    'estimated_duration': {
      'en': 'Estimated Duration (min)',
      'sw': 'Muda Unaokadiriwa (dak)',
    },
    'duration_placeholder': {'en': 'e.g. 15', 'sw': 'k.m. 15'},
    'no_minutes_yet': {
      'en': 'No minutes recorded',
      'sw': 'Hakuna muhtasari bado',
    },
    'no_minutes_desc': {
      'en': 'Record meeting minutes to capture discussions and decisions.',
      'sw': 'Andika muhtasari wa mkutano kuhifadhi majadiliano na maamuzi.',
    },
    'record_minutes': {'en': 'Record Minutes', 'sw': 'Andika Muhtasari'},
    'meeting_notes': {'en': 'Meeting Notes', 'sw': 'Maelezo ya Mkutano'},
    'minutes_placeholder': {
      'en': 'Write detailed meeting notes here...',
      'sw': 'Andika maelezo ya mkutano hapa...',
    },
    'summary': {'en': 'Summary', 'sw': 'Muhtasari'},
    'summary_placeholder': {
      'en': 'Brief summary...',
      'sw': 'Muhtasari mfupi...',
    },
    'key_decisions': {'en': 'Key Decisions', 'sw': 'Maamuzi Muhimu'},
    'decisions_placeholder': {
      'en': 'List key decisions...',
      'sw': 'Orodhesha maamuzi muhimu...',
    },
    'action_items': {'en': 'Action Items', 'sw': 'Hatua za Kufanya'},
    'action_items_placeholder': {
      'en': 'List action items...',
      'sw': 'Orodhesha hatua za kufanya...',
    },
    'save_minutes': {'en': 'Save Minutes', 'sw': 'Hifadhi Muhtasari'},
    'minutes_saved': {'en': 'Minutes saved', 'sw': 'Muhtasari umehifadhiwa'},
    'recorded_by': {'en': 'Recorded by', 'sw': 'Imeandikwa na'},
    'export_pdf': {'en': 'Export PDF', 'sw': 'Hamisha PDF'},
  };

  static String tr(String key, String locale) {
    return _translations[key]?[locale] ?? _translations[key]?['en'] ?? key;
  }

  static Map<String, String> allForLocale(String locale) {
    return _translations.map(
      (key, value) => MapEntry(key, value[locale] ?? value['en'] ?? key),
    );
  }
}

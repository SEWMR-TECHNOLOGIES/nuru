...
class _EventRsvpTabState extends State<EventRsvpTab> with AutomaticKeepAliveClientMixin {
    ...
    return Column(children: [
      ...
      Padding(
        ...
        child: SizedBox(
          ...
          child: OutlinedButton.icon(
            ...
            label: Text('RSVP Report', style: _f(size: 12, weight: FontWeight.w600)),
      ...
      Expanded(
        child: _loading
            ...
            : _guests.isEmpty
                ? Center(child: Text('No RSVP responses yet', style: _f(size: 14, color: AppColors.textTertiary)))
                : RefreshIndicator(
  ...
  Widget _rsvpTile(Map<String, dynamic> g) {
    final name = _str(g['name'], fallback: _str(g['full_name'], fallback: 'Guest'));
    final rsvp = _str(g['rsvp_status'], fallback: 'pending');
    ...
    final rsvpIcon = {'confirmed': Icons.check_circle_rounded, 'pending': Icons.access_time_rounded, 'declined': Icons.cancel_rounded, 'maybe': Icons.help_outline_rounded}[rsvp] ?? Icons.help_outline_rounded;
    ...
    return Container(
      ...
      child: Row(children: [
        CircleAvatar(
          radius: 20, backgroundColor: rsvpColor.withOpacity(0.1),
          child: Text(name.isNotEmpty ? name[0].toUpperCase() : '?', style: _f(size: 16, weight: FontWeight.w700, color: rsvpColor)),
        ),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(name, style: _f(size: 14, weight: FontWeight.w600), maxLines: 1, overflow: TextOverflow.ellipsis),
          const SizedBox(height: 2),
          Row(children: [
            Icon(rsvpIcon, size: 14, color: rsvpColor),
            const SizedBox(width: 4),
            Text(rsvp[0].toUpperCase() + rsvp.substring(1), style: _f(size: 12, weight: FontWeight.w600, color: rsvpColor)),
            if (phone.isNotEmpty) ...[const SizedBox(width: 8), Flexible(child: Text(phone, style: _f(size: 11, color: AppColors.textTertiary), overflow: TextOverflow.ellipsis))],
          ]),
          if (email.isNotEmpty) Text(email, style: _f(size: 11, color: AppColors.textTertiary), overflow: TextOverflow.ellipsis),
        ])),
      ]),
    );
  }
  ...
  Widget _statCard(String label, dynamic count, Color color) {
    return Container(
      ...
      child: Column(children: [
        Text('${count ?? 0}', style: _f(size: 20, weight: FontWeight.w800, color: color)),
        const SizedBox(height: 2),
  ...
  Future<void> _generateReport() async {
    ...
    final res = await ReportGenerator.generateRsvpReport(
      widget.eventId, format: 'pdf', guests: _guests,
    );
    if (!mounted) return;
    if (res['success'] == true && res['bytes'] != null) {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ReportPreviewScreen(
        title: 'RSVP Report', pdfBytes: res['bytes'] as Uint8List, filePath: res['path'] as String?,
      )));
    } else {

...
def list_user_events(
    ...
    try:
        ...
        if status != "all":
            ...
            # Handle enum vs string comparison safely
            if hasattr(EventStatusEnum, mapped_status):
                query = query.filter(Event.status == getattr(EventStatusEnum, mapped_status))
            else:
...
async def create_event(
    ...
    try:
        ...
        # Parse separate time field if provided
        parsed_time = None
        ...
        new_event = Event(
            ...
            start_time=parsed_time if parsed_time else parsed_start.time(),
    ...
    try:
        ...
        if start_date is not None:
            try:
                ...
                # Only set time from start_date if no separate time field provided
                if time is None:
                    event.start_time = ps.time()
            except ValueError:
                errors.append({"field": "start_date", "message": "Invalid ISO 8601 format."})

        # Handle separate time field
        if time is not None:
            if time.strip():
                try:
                    parts = time.strip().split(":")
                    if len(parts) == 2:
                        event.start_time = datetime.strptime(time.strip(), "%H:%M").time()
                    else:
                        event.start_time = datetime.strptime(time.strip(), "%H:%M:%S").time()
                except ValueError:
                    errors.append({"field": "time", "message": "Invalid time format. Use HH:MM or HH:MM:SS."})
        ...
        if images:
            ...
            if real_images:
                ...
                if existing_count + len(real_images) > MAX_EVENT_IMAGES:
                    db.rollback()
                    return standard_response(False, f"Total images would exceed the maximum of {MAX_EVENT_IMAGES}. Currently have {existing_count}.")
                for file in real_images:
...
# =============================================================================
# 5.6 POST /events/committee/accept — Accept invite (Public)
# =============================================================================

@router.post("/committee/accept")
def accept_committee_invitation(
    body: dict = Body(...),
    db: Session = Depends(get_db),
):
    # This endpoint is public (no auth required initially)
    # If user_id is provided, we link it.
    
    # Simulate token verification (in real app, decode JWT token from email)
    # For now, we'll assume token is valid or look up by some other means if needed.
    # Since we don't have an 'invitation_token' column on EventCommitteeMember in the provided schema,
    # we'll implement a placeholder that would verify the token.
    ...
    # 1. Decode token to get member_id or look up invitation
    # 2. Update status to 'active'
    # 3. Link user_id if provided

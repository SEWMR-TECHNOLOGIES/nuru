...
class UserProfile(Base):
    ...
    # User-selected interests captured during onboarding (chip-based picker).
    # Stored as a JSON array of slug strings, e.g. ["weddings","tech"].
    interests = Column(JSONB, nullable=False, server_default="'[]'::jsonb")
    # Why the user signed up — first onboarding question. Stored as a JSON
    # array of slug strings, e.g. ["plan_event","buy_tickets"].
    signup_intents = Column(JSONB, nullable=False, server_default="'[]'::jsonb")
    # How the user typically engages with events (attendee/host/planner/vendor).
    event_role = Column(Text, nullable=True)
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
...
class UserSetting(Base):
    ...
    rsvp_notifications = Column(Boolean, default=True)
    contribution_notifications = Column(Boolean, default=True)
    mention_notifications = Column(Boolean, default=True)
    marketing_emails = Column(Boolean, default=False)
    weekly_digest = Column(Boolean, default=True)
    quiet_hours_enabled = Column(Boolean, default=False)
    quiet_hours_start = Column(Text, default='22:00')
    quiet_hours_end = Column(Text, default='07:00')
    ...
    currency = Column(Text, default='TZS')

...
export const generateContributionReport = (
  ...
  const totalPledged = contributors.reduce((s, c) => s + c.pledged, 0);
  const totalPaid = contributors.reduce((s, c) => s + c.paid, 0);
  // Outstanding = budget - total raised (if budget exists), otherwise pledged - paid
  const outstanding = summary.budget ? Math.max(0, summary.budget - totalPaid) : contributors.reduce((s, c) => s + c.balance, 0);
  ...
  const html = `
    ...
    <body>
      ...
      <div class="summary">
        ...
        ${summary.target_amount ? `<div class="summary-card"><div class="label">Contribution Target</div><div class="value">${fmt(summary.target_amount)}</div></div>` : ''}
      ...
      <table>
        ...
        <tfoot>
          <tr>
            <td colspan="2">Total (${contributors.length} contributors)</td>

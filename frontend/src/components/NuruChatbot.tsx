...
// Detects when AI is asking for structured input (form-like data collection)
interface FormField {
  label: string;
  key: string;
  type: 'text' | 'select';
...
function isChoiceQuestion(content: string): boolean {
  ...
  // If the message is asking the user to PICK one of the listed options (not fill values)
  const choicePatterns = [
    /would you like to/i,
    /which (?:one|option) (?:do you|would you)/i,
    /what (?:feels|sounds|would you) like/i,
    /are you interested in/i,
    /let me know what(?:'s| is) on your mind/i,
    /what(?:'s| is) (?:the )?best next step/i,
    /choose one/i,
    /pick one/i,
    /select (?:one|an option)/i,
    /what kind of help/i,
    /how can i help/i,
    /what do you(?:'d)? (?:want|prefer|think)/i,
  ];
  return choicePatterns.some(p => p.test(lower));
}

function looksLikeDataInput(label: string): boolean {
  // Items that need the user to FILL IN a value (budget, count, name, date, amount)
  const dataPatterns = [
    /budget/i, /cost/i, /price/i, /amount/i, /how much/i,
    /count/i, /number of/i, /how many/i,
    /date/i, /when/i, /time/i,
    /name/i, /title/i, /theme/i,
    /location/i, /venue/i, /where/i,
    /type of event/i, /event type/i, /kind of event/i,
    /guest/i, /attendee/i,
    /rental/i, /catering/i, /decoration/i, /entertainment/i,
    /photography/i, /videography/i, /cake/i, /invitation/i,
    /outfit/i, /favor/i, /transport/i, /music/i,
  ];
  return dataPatterns.some(p => p.test(label));
}

function extractFormFields(content: string): FormField[] | null {
  // If AI is presenting choices for user to pick from, don't show a form
  if (isChoiceQuestion(content)) return null;

  const lower = content.toLowerCase();
  
  // Check if the AI is asking for multiple pieces of information to fill in
  const needsInfoPattern = /(?:i(?:'ll| will) need|please (?:provide|share|tell)|could you (?:provide|share|tell)|i need (?:to know|the following)|share the following|provide the following|fill in|here(?:'s| is) (?:a |the )?(?:form|template|breakdown)|let(?:'s| us) (?:start|begin|set up|break down))/i;
  if (!needsInfoPattern.test(lower)) return null;
  ...
  for (const line of lines) {
    ...
    if (match) {
      let label = match[1].trim().replace(/\?+$/, '').replace(/\*\*/g, '').trim();
      // Remove parenthetical hints
      label = label.replace(/\s*\([^)]*\)/, '').trim();

      if (label.length < 3 || label.length > 60) continue;

      // Only include if it looks like a data-entry field, not a choice
      if (!looksLikeDataInput(label)) continue;

      const key = label.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '');

      // Detect if it should be a select with known options
      if (/event\s*type|type of event|kind of event/i.test(label)) {
        fields.push({ label, key, type: 'select', options: ['Wedding', 'Birthday', 'Graduation', 'Corporate', 'Memorial', 'Other'] });
      } else if (/location|city|area|venue|where/i.test(label)) {
        fields.push({ label, key, type: 'select', options: ['Dar es Salaam', 'Arusha', 'Dodoma', 'Mwanza', 'Zanzibar', 'Other'] });
      } else {
              ...
              <CardContent className="p-0 flex flex-col flex-1 min-h-0">
                ...
                >
                  ...
                  {!isLoading && messages.length > 1 && messages[messages.length - 1]?.role === 'assistant' && (() => {
                    ...
                    // Check for form fields first (structured input)
                    const formFields = extractFormFields(lastContent);
                    if (formFields) {
                      return <InlineForm fields={formFields} onSubmit={(values) => {
...
// -- StepWizard: one question at a time, carousel-style --
function InlineForm({ fields, onSubmit }: { fields: FormField[]; onSubmit: (values: Record<string, string>) => void }) {
  ...
  const totalAnswered = Object.values(values).filter(v => v.trim()).length;

  const handleSelect = (key: string, opt: string) => {
    setValues(prev => ({ ...prev, [key]: prev[key] === opt ? '' : opt }));
    setShowOtherInput(prev => ({ ...prev, [key]: false }));
  };

  const handleOtherToggle = (key: string) => {
    const showing = !showOtherInput[key];
    setShowOtherInput(prev => ({ ...prev, [key]: showing }));
    if (!showing) {
      setValues(prev => ({ ...prev, [key]: '' }));
      setOtherText(prev => ({ ...prev, [key]: '' }));
    } else {
      setValues(prev => ({ ...prev, [key]: otherText[key] || '' }));
    }
  };

  const handleOtherText = (key: string, text: string) => {
    setOtherText(prev => ({ ...prev, [key]: text }));
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-1">
        ...
        <p className="text-[10px] text-muted-foreground">{totalAnswered} answered</p>
      </div>
        ...
        {fields.map((_, i) => (
          <div
            ...
            className={`h-1 flex-1 rounded-full transition-colors ${
              i < step ? 'bg-foreground' : i === step ? 'bg-foreground/60' : 'bg-border'
            }`}
          />
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          exit={{ opacity: 0, x: -20 }}
          transition={{ duration: 0.15 }}
          className="bg-muted/50 rounded-xl border border-border p-3 space-y-2.5"
        >
          <label className="text-xs font-medium text-foreground/80 capitalize block">
            {field.label}
          </label>
          ...
          {field.type === 'select' && field.options ? (
            <div className="space-y-2">
              <div className="flex flex-wrap gap-1.5">
                {field.options.map((opt) => (
                  <button
                    ...
                    className={`text-[11px] px-2.5 py-1.5 rounded-full border transition-colors ${
                      values[field.key] === opt && !showOtherInput[field.key]
                ...
                <button
                  onClick={() => handleOtherToggle(field.key)}
                  className={`text-[11px] px-2.5 py-1.5 rounded-full border transition-colors ${
                    showOtherInput[field.key]
                      ? 'bg-foreground text-background border-foreground'
              ...
              {showOtherInput[field.key] && (
                <motion.input
                  ...
                  onChange={(e) => handleOtherText(field.key, e.target.value)}
                  placeholder="Type your answer..."
                  autoFocus
                  className="w-full text-xs bg-background rounded-lg border border-border px-2.5 py-1.5 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-foreground/20"
                />
              )}
            </div>
          ) : (
      ...
      <div className="flex gap-2 px-1">
        ...
        <Button
          ...
          size="sm"
          onClick={goNext}
          className="h-7 text-[11px] rounded-lg px-3 text-muted-foreground"
        >
          Skip
        </Button>
        <Button
          ...
          className="h-7 text-[11px] rounded-lg px-3 bg-foreground text-background hover:bg-foreground/90 ml-auto"
        >
          {isLast ? `Send (${totalAnswered})` : 'Next'}
        </Button>
      </div>
    </motion.div>
  );
